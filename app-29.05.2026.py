    ...
    try:
        ...
        if reminderNotes:
            for reminderNote in reminderNotes:
                ...
                note = reminderNote[1]
                studentName = f"{reminderNote[2] or ''} {reminderNote[3] or ''}".strip()
        ...
        try:
            ...
            if nationality == "nepalese":
                htmlContent = f"<p>Hi UAN,</p><p>You have a new student registration</p><p>Student Name :- {firstName} {lastName}<br>Student id:- {last_insert_id}" \
                              f"<br>Mobile :- {phone}<br>Email :- {email} <br>Nationality :- {nationality}</p>" \
                              f"<p>Thanks<br>UAN Team</p>"
                sendMail('Registration form Student Added', htmlContent, email)
            elif nationality == "indian":
...
class addNewApplication(Resource):
    def post(self):
        ...
        try:
            ...
            value = (
                studentId, courseType, institutionName, courseStartDate, courseEndDate, courseName,
                dateOfApplicationSent,
...
class getApplicationCSDReport(Resource):
    def post(self):
        ...
        statusList = studentStatus.split(",")
        applicationReport = []
        # cnx = mysql.connect(user=dbUser, password=dbPassword, database=dataBase)
        cnx, cursor = get_connection_and_cursor()
        try:
            if officeId != "" and "All" in courseTypes and "All" in institutionIds and "All" in statusList:
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                    ...
                                    date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                    left outer join students on students.student_id=application.student_id 
                                    where application.course_start_date BETWEEN %s AND %s and office_id=%s"""
                value = (fromDate, toDate, officeId,)
            elif officeId == "" and "All" not in courseTypes and "All" in institutionIds and "All" in statusList:
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                    last_name, mobile_no, nationality, reference_no, course_type, course_start_date, 
                                    date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                    left outer join students on students.student_id=application.student_id 
            ...
            elif officeId == "" and "All" in courseTypes and "All" not in institutionIds and "All" in statusList:
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                    last_name, mobile_no, nationality, reference_no, course_type, course_start_date, 
                                    date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                    left outer join students on students.student_id=application.student_id 
            ...
            elif officeId == "" and "All" in courseTypes and "All" in institutionIds and "All" not in statusList:
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                    last_name, mobile_no, nationality, reference_no, course_type, course_start_date, 
                                    date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                    left outer join students on students.student_id=application.student_id 
                                    where application.course_start_date BETWEEN %s AND %s and students.status IN ({})"""
                statusJoinData = ', '.join(['%s'] * len(statusList))

                sqlSelectAppReport = sqlSelectAppReport.format(statusJoinData)

                value = [fromDate, toDate] + statusList
            elif officeId == "" and "All" not in courseTypes and "All" not in institutionIds and "All" in statusList:
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                    last_name, mobile_no, nationality, reference_no, course_type, course_start_date, 
                                    date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                    left outer join students on students.student_id=application.student_id 
            ...
            elif officeId == "" and "All" not in courseTypes and "All" in institutionIds and "All" not in statusList:
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                    last_name, mobile_no, nationality, reference_no, course_type, course_start_date, 
                                    date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                    left outer join students on students.student_id=application.student_id 
                                    where application.course_start_date BETWEEN %s AND %s and course_type IN ({}) and students.status IN ({})"""
                courseTypeJoinData = ', '.join(['%s'] * len(courseTypes))
                statusJoinData = ', '.join(['%s'] * len(statusList))

                sqlSelectAppReport = sqlSelectAppReport.format(courseTypeJoinData, statusJoinData)

                value = [fromDate, toDate] + courseTypes + statusList
            elif officeId == "" and "All" in courseTypes and "All" not in institutionIds and "All" not in statusList:
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                             last_name, mobile_no, nationality, reference_no, course_type, course_start_date, 
                                             date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                             left outer join students on students.student_id=application.student_id 
                                             where application.course_start_date BETWEEN %s AND %s and institution_id IN ({}) and students.status IN ({})"""
                institutionIdJoinData = ', '.join(['%s'] * len(institutionIds))
                statusJoinData = ', '.join(['%s'] * len(statusList))
                ...
                value = [fromDate, toDate] + institutionIds + statusList
            elif officeId == "" and "All" not in courseTypes and "All" not in institutionIds and "All" not in statusList:
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                    last_name, mobile_no, nationality, reference_no, course_type, course_start_date, 
                                    date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                    left outer join students on students.student_id=application.student_id 
                                    where application.course_start_date BETWEEN %s AND %s and course_type IN ({}) and institution_id IN ({}) and students.status IN ({})"""
                institutionIdJoinData = ', '.join(['%s'] * len(institutionIds))
                courseTypeJoinData = ', '.join(['%s'] * len(courseTypes))
                statusJoinData = ', '.join(['%s'] * len(statusList))

                sqlSelectAppReport = sqlSelectAppReport.format(courseTypeJoinData, institutionIdJoinData, statusJoinData)

                value = [fromDate, toDate] + courseTypes + institutionIds + statusList
            elif officeId != "" and "All" not in courseTypes and "All" not in institutionIds and "All" not in statusList:
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                    last_name, mobile_no, nationality, reference_no, course_type, course_start_date, 
                                    date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                    ...
                                    institution_id IN ({}) and students.status IN ({})"""
                institutionIdJoinData = ', '.join(['%s'] * len(institutionIds))
                courseTypeJoinData = ', '.join(['%s'] * len(courseTypes))
                statusJoinData = ', '.join(['%s'] * len(statusList))
                ...
                value = [fromDate, toDate, officeId] + courseTypes + institutionIds + statusList
            elif officeId != "" and "All" not in courseTypes and "All" in institutionIds and "All" in statusList:
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                    last_name, mobile_no, nationality, reference_no, course_type, course_start_date, 
                                    date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                ...
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                               ...
                                               date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                               left outer join students on students.student_id=application.student_id 
                                               where application.course_start_date BETWEEN %s AND %s and office_id=%s  and 
                                               institution_id IN ({}) and students.status IN ({})"""
                institutionIdJoinData = ', '.join(['%s'] * len(institutionIds))
                statusJoinData = ', '.join(['%s'] * len(statusList))
                ...
                value = [fromDate, toDate, officeId] +  institutionIds + statusList
            elif officeId != "" and "All" in courseTypes and "All" in institutionIds and "All" not in statusList:
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                               last_name, mobile_no, nationality, reference_no, course_type, course_start_date, 
                                               date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                               left outer join students on students.student_id=application.student_id 
                                               where application.course_start_date BETWEEN %s AND %s and office_id=%s and students.status IN ({})"""
                statusJoinData = ', '.join(['%s'] * len(statusList))

                sqlSelectAppReport = sqlSelectAppReport.format(
                                                               statusJoinData)

                value = [fromDate, toDate, officeId] + statusList
            elif officeId != "" and "All" not in courseTypes and "All" not in institutionIds and "All" in statusList:
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                               last_name, mobile_no, nationality, reference_no, course_type, course_start_date, 
                                               date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
        ...
        try:
            if officeId != "" and "All" in courseTypes and "All" in institutionIds and "All" in statusList:
                ...
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                               ...
                                               date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                               left outer join students on students.student_id=application.student_id 
                                               where application.date_of_application_sent BETWEEN %s AND %s and students.status IN ({})"""
                statusJoinData = ', '.join(['%s'] * len(statusList))
                ...
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                               ...
                                               date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                               left outer join students on students.student_id=application.student_id 
                                               where application.date_of_application_sent BETWEEN %s AND %s and course_type IN ({}) and students.status IN ({})"""
                courseTypeJoinData = ', '.join(['%s'] * len(courseTypes))
                statusJoinData = ', '.join(['%s'] * len(statusList))
                ...
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                               ...
                                               date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                               left outer join students on students.student_id=application.student_id 
                                               where application.date_of_application_sent BETWEEN %s AND %s and institution_id  IN ({}) and students.status IN ({})"""
                institutionIdJoinData = ', '.join(['%s'] * len(institutionIds))
                statusJoinData = ', '.join(['%s'] * len(statusList))
                ...
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                    ...
                                    date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                    left outer join students on students.student_id=application.student_id 
                                    where application.date_of_application_sent BETWEEN %s AND %s and course_type IN ({}) and institution_id IN ({}) and students.status IN ({})"""
                institutionIdJoinData = ', '.join(['%s'] * len(institutionIds))
                courseTypeJoinData = ', '.join(['%s'] * len(courseTypes))
                statusJoinData = ', '.join(['%s'] * len(statusList))
                ...
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                    ...
                                    date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                    left outer join students on students.student_id=application.student_id 
                                    where application.date_of_application_sent BETWEEN %s AND %s and office_id=%s and institution_id IN ({}) and students.status IN ({})"""
                institutionIdJoinData = ', '.join(['%s'] * len(institutionIds))
                statusJoinData = ', '.join(['%s'] * len(statusList))

                sqlSelectAppReport = sqlSelectAppReport.format(institutionIdJoinData, statusJoinData)
                ...
                sqlSelectAppReport = """select students.student_id as student_id, application_id, institution_name, first_name, 
                                    ...
                                    date_of_application_sent, offer_decision_type, tution_fee, students.status as status from application 
                                    left outer join students on students.student_id=application.student_id 
                                    where application.date_of_application_sent BETWEEN %s AND %s and office_id=%s and students.status IN ({})"""
                statusJoinData = ', '.join(['%s'] * len(statusList))
    ...
    def post(self):
        ...
        try:
            ...
            return jsonify({"success": "Success",
                            "message": "Your form has been successfully submitted. A unique code has been sent to your email. Please check your inbox",
                            "unique_id": unique_id})
        except mysql.Error as e:
