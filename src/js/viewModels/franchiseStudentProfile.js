    ...
    function (oj,ko,$, app, ArrayDataProvider) {
        ...
        class FranchiseStudentProfile {
            constructor(args) {
                ...
                self.franchiseList = ko.observableArray()
                self.franchises = (officeId)=>{
                    $.ajax({
                        url: BaseURL+"/getFranchiseWithOfficeId",
                        type: 'POST',
                        data: JSON.stringify({
                            officeId: officeId,
                        }),
                        ...
                        success: function (data) {
                            if(data[0]!='No data found'){
                                ...
                                let len = data.length;
                                for(let i=0;i<len;i++){
                                    self.franchiseList.push({value: `${data[i][0]}`, label: `${data[i][1]}`})
                                }
                            }
                        }
                    })
                }
                self.franchisesDp = new ArrayDataProvider(self.franchiseList, {
                    keyAttributes: 'value'
                });
                ...
                self.abroadDestionations = [
                    ...
                    {"label":"European countries","value":"European"},
                    {"label":"UAE","value":"UAE"}
                ]
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if(data[0] != "No data found"){
                                ...
                                self.franchiseName(data[0][35] + " " + data[0][36]);
        ...
        return  FranchiseStudentProfile;
    }
);